...
ggplot() +
  ...
  geom_sf(data = (survey_sf %>%  filter(str_starts(survey_location, "1"))), aes(color = group), size = 3) +           # Survey locations, colored by group
  scale_color_manual(values = c("1" = "yellow", 
                                "2" = "orange", 
                                "3" = "blue", 
                                "4" = "purple", 
                                "5" = "darkgreen")) + # Customize colors for groups
  theme_bw() +
...
pal <- colorFactor(palette = c("yellow", "cyan", "steelblue", "navy", "darkgreen"), domain = survey_sf$group)
