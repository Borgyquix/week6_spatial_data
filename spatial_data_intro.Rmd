...
ggplot() +
  geom_sf(data = sb_county, fill = "lightgray", color = "black") + # Plot the county map
...
leaflet() %>%
  ...
  addPolygons(data = sb_county, color = "black", weight = 1, fillColor = "lightgray") %>% # Add county boundaries
  addCircleMarkers(data = survey_sf, radius = 5, color = "blue", popup = ~paste("Site:", survey_location)) %>%
